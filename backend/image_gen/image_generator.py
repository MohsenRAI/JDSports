...
from image_gen.prompts import get_body_generation_prompt, get_outfit_application_prompt, get_arcticfox_body_generation_prompt, get_heather_body_generation_prompt, get_black_body_generation_prompt
...
⚠️ CRITICAL: If ANY of these checks fail, the image must be regenerated."""
        ...
        self.full_body_transform_prompt = '''Create a professional fashion photo of a male model with the specified body type and skin color, using the attached reference image for the exact hoodie style, fit, Jumpman logo placement, and color (RGB: #C70024 – a rich, vibrant red).
...
The model's head, ALL hair, and the ENTIRE hoodie (including the full bottom hem and both sleeves from shoulder to wrist) must be completely visible in the frame

Do not crop, blur, hide, or omit any part of the hoodie

The white Jumpman logo on the chest must be clearly visible, centered, unobstructed, and identical in shape and position to the attached reference image

The entire front panel of the hoodie must be clearly seen and well lit
...
The full hoodie (bottom hem + both sleeves) must remain fully inside the visible frame and must NOT touch or cross any frame edges

CAMERA SETUP AND POSITION:

Camera at chest height (~4.5 feet / 137cm from ground)

Slight upward tilt (5–10 degrees) or level angle

Focal length: 85mm equivalent (for natural proportions)

Portrait orientation (4:5 ratio)

Framed to show from top of the head to just below the hips

POSE AND STYLING:

Model is standing naturally, facing the camera, in a relaxed pose

Arms at sides or lightly interacting with the hoodie (e.g., adjusting pocket or sleeve)

Face clearly visible, with a neutral or confident expression

Head and upper torso must be clearly framed and unobstructed

BACKGROUND AND LIGHTING:

Outdoor urban sidewalk setting in New York City

Include soft-focus background elements like red-brick buildings, concrete pavement, and storefronts

Use bright diffused daylight (e.g., overcast or golden hour)

Lighting must be natural, evenly distributed, and consistent with real-world fashion photography

CLOTHING SPECIFICATIONS:

Bright red Jordan hoodie with white Jumpman logo on chest

Hoodie must be untucked, worn naturally, with no distortions or fabric alterations

Match the exact color tone from reference (RGB: #C70024)

No visible branding or accessories beyond the hoodie itself

TECHNICAL REQUIREMENTS:

8K quality

Hyperrealistic and photorealistic

Sharp focus throughout (especially on face and garment)
...
FINAL CHECKS — ALL MUST BE TRUE:

✅ Head centered in top middle grid cell with full space above
✅ Entire head, hair, and hoodie (hem + sleeves) visible and uncropped
✅ Jumpman logo is accurate, centered, sharp, and unobstructed
✅ Front panel of hoodie clearly visible
✅ Urban sidewalk setting and natural light look realistic
✅ Shot must be discarded and regenerated if ANY of the above is missing'''

        # Define the transform prompt for knee-length (2/3 body)
        self.knee_length_transform_prompt = """Create a knee-length (2/3 body) professional fashion photo by transforming this base body image to wear the exact clothing from the reference fashion image.

CRITICAL REQUIREMENTS (MUST FOLLOW EXACTLY):
- ⚠️ DON'T ADD ANY CLOTHING ITEMS other than the ones in the reference image. DON'T ADD JACKET if the reference image doesn't have one
...
- ⚠️ CRITICAL: Keep all shirts UNTUCKED exactly as shown in the reference images - shirts should hang naturally outside pants/trousers, not tucked in
- ⚠️ CRITICAL: Pay EXTREMELY close attention to the EXACT COLOR of the shirt from the reference image - this must be perfectly preserved as it's the main product being sold
...
CRITICAL INSTRUCTIONS: COPY THE EXACT CLOTHING FROM THE REFERENCE IMAGE and don't add any additional items like sunglasses, hats, etc.
Add proper lighting and SHADOWS to the image to make it look realistic using the background setting from the pose description
"""
...
NOTE: Maintain the exact COLORS and TEXTURE of the clothing from the reference image. Apply the critical instructions for clothing details exactly and carefully and consistently as stated above.
Don't add any additional items like sunglasses, hats, etc."""
    ...
    def generate_body_variation(
        ...
        try:
            ...
            # Select the appropriate body prompt based on color
            use_arcticfox = False
            use_heather = False
            use_black = False
            if output_file:
                if output_file.startswith("arcticfox_"):
                    use_arcticfox = True
                elif output_file.startswith("heather_"):
                    use_heather = True
                elif output_file.startswith("black_"):
                    use_black = True
            elif hasattr(self, 'color_prefix'):
                if self.color_prefix == 'arcticfox_':
                    use_arcticfox = True
                elif self.color_prefix == 'heather_':
                    use_heather = True
                elif self.color_prefix == 'black_':
                    use_black = True

            if use_arcticfox:
                prompt = get_arcticfox_body_generation_prompt(body_type, body_description)
            elif use_heather:
                prompt = get_heather_body_generation_prompt(body_type, body_description)
            elif use_black:
                prompt = get_black_body_generation_prompt(body_type, body_description)
            else:
...
Generate a clean, professional base body image suitable for fashion photography."""
...
"""
            ...
            # This avoids the images.edit() API call that doesn't work with your API key
            self._generate_image_with_prompt(transform_prompt, output_file)
    ...
    def _generate_image_with_prompt(self, prompt, output_file=None, size="1024x1024", quality="medium"):
        """Generate an image using the OpenAI API with the given prompt."""
        try:
            response = self.client.images.generate(
                model=IMAGE_MODEL,
                prompt=prompt,
            ...
            # Get the image data - gpt-image-1 returns base64 data, not URLs
            image_data = response.data[0].b64_json
            
            # Check if we got valid image data
            if not image_data:
                print(f"Error: No image data returned from API")
                print(f"Response data: {response.data}")
                raise Exception("No image data returned from API")
            
            # If output_file is provided, save the base64 image
            if output_file:
                import base64
                from PIL import Image
                import io
    ...
    def _apply_outfit_from_reference(self, base_image_url, reference_image_url):
        """Apply the outfit from the reference image to the base image."""
        try:
            # Load the base image
            base_response = requests.get(base_image_url)
            base_image = Image.open(BytesIO(base_response.content))
            
            # Load the reference image
            ref_response = requests.get(reference_image_url)
            ref_image = Image.open(BytesIO(ref_response.content))
            
            # Create a prompt for applying the outfit
            outfit_prompt = f"""Create a photorealistic image of the man wearing the complete outfit from the reference photo.
...
- REPRODUCE the exact garment type (quarter-zip pullover sweater) as shown in the reference
- Maintain the same fit, patterns, textures and color of clothing
- Ensure clothing fits the body type appropriately
- Preserve all product details, zipper, logo, etc.
- ⚠️ CRITICAL: Keep the quarter-zip pullover sweater UNTUCKED exactly as shown in the reference images
- ⚠️ CRITICAL: Pay EXTREMELY close attention to the EXACT COLOR of the quarter-zip pullover sweater from the reference image - this must be perfectly preserved as it's the main product being sold"""
            
            # Generate the final image with the outfit applied
            final_image_url = self._generate_image_with_prompt(outfit_prompt)
            return final_image_url
        except Exception as e:
            print(f"Error applying outfit: {str(e)}")
            raise
...
- ENSURE all seams, stitching, buttons, and hardware elements are accurately recreated
- REPRODUCE any fabric-specific qualities like wrinkle patterns, drape, or stretch characteristics
- MAINTAIN the exact same fabric thickness and weight appearance
- APPLY these fabric details consistently across the entire garment
"""
    ...
    def _get_detailed_skin_description(self, skin_color):
        ...
        skin_descriptions = {
            ...
            "dark-brown": "Deep brown skin with rich chocolate tones, warm reddish undertones, and beautiful natural sheen",
        }
